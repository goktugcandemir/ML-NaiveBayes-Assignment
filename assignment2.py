...
def myNaiveBayesForCategoricalData(sentencestoPredict):
    PoBeingBook,PoBeingMusic,PoBeingHealth,PoBeingDvd,PoBeingCamera,PoBeingSoftware,totalWordCount = getProbabilityForCategoricalData(y_train)
    ...
    result=max(probabilityOfBeingBook,probabilityOfBeingMusic,probabilityOfBeingHealth,probabilityOfBeingDvd,probabilityOfBeingCamera,probabilityOfBeingSoftware)
...
data = pd.read_csv("all_sentiment_shuffled.txt",sep = "\t",names=['tokens','topic','sentiment','document identifier'])
